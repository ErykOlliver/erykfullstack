...
export default function Quest({ isOpen, onToggle, question, response }: QuestProps) {
    return (
        <div className={`w-full md:w-5/6 h-fit transition-all duration-100 rounded-md`}>
            <div
                ...
                onClick={onToggle}
            >
                <Paragraph className='font-medium md:text-sm xl:text-lg'>
                    {question}
                </Paragraph>
            ...
            {isOpen && (
                <div className='p-3 border border-primary-500 border-t-0 rounded-b-md'>
                    <Paragraph className='font-normal text-black-600/80 md:text-sm xl:text-lg'>{response}</Paragraph>
                </div>
            )}
        </div>
    )
